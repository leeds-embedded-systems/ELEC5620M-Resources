...
static HpsErr_t _FPGA_IrDA_getInterruptFlags(FPGAIrDACtx_t* ctx, FPGAIrDAIrqSources mask, bool clear) {
    // Read in the flags
...
static unsigned int _FPGA_IrDA_writeSpace(FPGAIrDACtx_t* ctx) {
    return FPGAIRDA_REG_TXSPACE(ctx->base);
}

static HpsErr_t _FPGA_IrDA_write(FPGAIrDACtx_t* ctx, const uint8_t* data, uint8_t length) {
    int written = 0;
    if (!length) return 0;
...
static unsigned int _FPGA_IrDA_available(FPGAIrDACtx_t* ctx) {
    return FPGAIRDA_REG_RXAVAIL(ctx->base);
}

static void _FPGA_IrDA_readWord(FPGAIrDACtx_t* ctx, UartRxData_t* data) {
...
static HpsErr_t _FPGA_IrDA_read(FPGAIrDACtx_t* ctx, uint8_t* data, uint8_t length) {
...
static void _FPGA_IrDA_cleanup(FPGAIrDACtx_t* ctx) {
...
static HpsErr_t _FPGA_IrDA_txFifoSpace(FPGAIrDACtx_t* ctx) {
    // Ensure context valid and initialised
    HpsErr_t status = DriverContextValidate(ctx);
    if (ERR_IS_ERROR(status)) return status;
    // Return FIFO space
...
static HpsErr_t _FPGA_IrDA_rxFifoAvailable(FPGAIrDACtx_t* ctx) {
    // Ensure context valid and initialised
    HpsErr_t status = DriverContextValidate(ctx);
    if (ERR_IS_ERROR(status)) return status;
...
static HpsErr_t _FPGA_IrDA_txIdle(FPGAIrDACtx_t* ctx, bool clearFlag) {
    // Ensure context valid and initialised
    HpsErr_t status = DriverContextValidate(ctx);
    if (ERR_IS_ERROR(status)) return status;
...
static HpsErr_t _FPGA_IrDA_rxReady(FPGAIrDACtx_t* ctx, bool clearFlag) {
    // Ensure context valid and initialised
    HpsErr_t status = DriverContextValidate(ctx);
    if (ERR_IS_ERROR(status)) return status;
...
HpsErr_t FPGA_IrDA_initialise(void* csr, FPGAIrDACtx_t** pCtx) {
