...
// CSR Registers
#define FPGAIRDA_REG_TXFIFO(base)     (*(((uint16_t*)base) + (0x00 / sizeof(uint16_t))))
#define FPGAIRDA_REG_RXFIFO(base)     (*(((uint16_t*)base) + (0x00 / sizeof(uint16_t))))
#define FPGAIRDA_REG_RXAVAIL(base)    (*(((uint8_t *)base) + (0x02 / sizeof(uint8_t ))))
#define FPGAIRDA_REG_IRQMASK(base)    (*(((uint8_t *)base) + (0x04 / sizeof(uint8_t ))))
#define FPGAIRDA_REG_IRQFLAGS(base)   (*(((uint8_t *)base) + (0x05 / sizeof(uint8_t ))))
#define FPGAIRDA_REG_TXSPACE(base)    (*(((uint8_t *)base) + (0x06 / sizeof(uint8_t ))))
#define FPGAIRDA_REG_CLEARFIFO(base)  (*(((uint8_t *)base) + (0x07 / sizeof(uint8_t ))))

// Register bits
#define FPGAIRDA_FIFO_DATA_MASK     0xFF // Both Tx and Rx FIFOs
#define FPGAIRDA_FIFO_DATA_OFFS     0
#define FPGAIRDA_RXFIFO_PARITY_MASK 0x1
#define FPGAIRDA_RXFIFO_PARITY_OFFS 9
#define FPGAIRDA_RXFIFO_VALID_MASK  0x1
#define FPGAIRDA_RXFIFO_VALID_OFFS  15
#define FPGAIRDA_CLEARFIFO_TX_MASK  0x1
#define FPGAIRDA_CLEARFIFO_TX_OFFS  0
#define FPGAIRDA_CLEARFIFO_RX_MASK  0x1
