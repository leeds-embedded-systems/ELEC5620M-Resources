...
static inline HpsErr_t GPIO_setDirection(PGpioCtx_t gpio, unsigned int dir, unsigned int mask) {
    if (!gpio) return ERR_NULLPTR;
    if (!gpio->setDirection) return ERR_NOSUPPORT;
    return gpio->setDirection(gpio->ctx,dir,mask);
}

static inline HpsErr_t GPIO_getDirection(PGpioCtx_t gpio, unsigned int* dir, unsigned int mask) {
    if (!gpio || !dir) return ERR_NULLPTR;
    if (!gpio->getDirection) return ERR_NOSUPPORT;
    return gpio->getDirection(gpio->ctx,dir,mask);
}

static inline HpsErr_t GPIO_setOutput(PGpioCtx_t gpio, unsigned int port, unsigned int mask) {
    if (!gpio) return ERR_NULLPTR;
    if (!gpio->setOutput) return ERR_NOSUPPORT;
    return gpio->setOutput(gpio->ctx,port,mask);
}

static inline HpsErr_t GPIO_getOutput(PGpioCtx_t gpio, unsigned int* port, unsigned int mask) {
    if (!gpio || !port) return ERR_NULLPTR;
    if (!gpio->getOutput) return ERR_NOSUPPORT;
    return gpio->getOutput(gpio->ctx,port,mask);
}

static inline HpsErr_t GPIO_toggleOutput(PGpioCtx_t gpio, unsigned int mask) {
    if (!gpio) return ERR_NULLPTR;
    if (!gpio->toggleOutput) return ERR_NOSUPPORT;
    return gpio->toggleOutput(gpio->ctx,mask);
}

static inline HpsErr_t GPIO_getInput(PGpioCtx_t gpio, unsigned int* in, unsigned int mask) {
