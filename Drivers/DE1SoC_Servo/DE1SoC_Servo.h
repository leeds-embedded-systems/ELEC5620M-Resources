...
// - "channel" is the servo channel to initialise the context for.
...
// - When the servo is disabled (see function above), the PWM pin becomes a general purpose input.
//   You can use this function to read the 1-bit input value.
// - Reads will populate *value with the pin value.
// - When the servo is enabled, the pin should not be used as an input.
...
HpsErr_t Servo_pulseWidthRange( ServoCtx_t* ctx, bool doubleWidth );
...
//   20ms or 40ms for many commonly used servos.
